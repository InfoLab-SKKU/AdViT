...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        layer_wise_tokens = [self.norm(x) for idx, x in enumerate(layer_wise_tokens)]
        layer_wise_tokens = [self.avgpool(x.transpose(1, 2)) for x in layer_wise_tokens]
        layer_wise_tokens = [torch.flatten(x, 1) for x in layer_wise_tokens]

        return layer_wise_tokens
